...
SecureMessageGUI::SecureMessageGUI(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::SecureMessageGUI),
    walletModel(0),
    platformStyle(platformStyle)
{
    ...
    connect(ui->user_list, &QListWidget::itemClicked, this, &SecureMessageGUI::populateConversation);
    connect(ui->user_list->itemDelegate(), &QAbstractItemDelegate::commitData, this, &SecureMessageGUI::renameAlias);
    connect(ui->user_list, &QListWidget::customContextMenuRequested, this, &SecureMessageGUI::userListContextMenu);
    ...
    // Check new messages and user contacts every second
    auto *timer = new QTimer(this);
    connect(timer, &QTimer::timeout, this, &SecureMessageGUI::checkForNewMessages);
    timer->start(1000);
}

SecureMessageGUI::~SecureMessageGUI()
{
    delete ui;
}

void SecureMessageGUI::setWalletModel(WalletModel *model)
{
...
void SecureMessageGUI::on_send_button_clicked()
{
    std::string message = ui->input_text->toPlainText().toStdString();
...
void SecureMessageGUI::on_choose_address_clicked()
{
...
void SecureMessageGUI::deleteConversation(std::string addrFrom)
{
    {
        LOCK(cs_smsgDB);
...
void SecureMessageGUI::on_remove_contact_clicked()
{
    QList<QListWidgetItem *> items = ui->user_list->selectedItems();
...
void SecureMessageGUI::on_clear_conversation_clicked()
{
    QList<QListWidgetItem *> items = ui->user_list->selectedItems();
...
void SecureMessageGUI::setSendingAddress()
{
    // Get first address book entry, should already be added for SsecureMsg by SecureMsgAddWalletAddresses()
    for (auto& entry : pwalletMain->mapAddressBook){
        sendingAddress = CBitcoinAddress{entry.first}.ToString();
        settingsAddress = sendingAddress;

        if (SecureMsgAddWalletAddress(sendingAddress)) {
            // Get matching pubkey
            SecureMsgGetLocalPublicKey(sendingAddress, sendingPubKey);

            // First valid address set as sending address, can break here.
            break;
        }
    }
...
void SecureMessageGUI::showEvent(QShowEvent* event)
{
...
void SecureMessageGUI::populateConversation()
{
...
void SecureMessageGUI::renameAlias(QWidget *editor)
{
    QList<QListWidgetItem *> items = ui->user_list->selectedItems();
...
void SecureMessageGUI::userListContextMenu(const QPoint& pos)
{
    QList<QListWidgetItem *> items = ui->user_list->selectedItems();
...
void SecureMessageGUI::editUserListItem()
{
    QList<QListWidgetItem *> items = ui->user_list->selectedItems();
...
void SecureMessageGUI::checkForNewMessages()
{
    std::set<Message, MessageCmp> messages;
...
void SecureMessageGUI::checkMessages(std::set<Message, MessageCmp>& messages, bool unread)
{
...
void SecureMessageGUI::addMessagesToConversation(std::set<Message, MessageCmp>& messages)
{
    for (auto& msg : messages) {
        ...
        if (sendingAddress == msg.to) {
            message_widget->setStyleSheet("QWidget { background: #fff; border-radius: 5px; margin-left: 9px; margin-top: 3px; }");
        } else {
            message_widget->setStyleSheet("QWidget { background: #effdde; border-radius: 5px;  margin-left: 9px; margin-top: 3px; }");
        }
...
void SecureMessageGUI::populateUserList()
{
    std::map<std::string, std::string> aliases;
    if (!SecureMsgGetAllAliases(aliases)) {
        QMessageBox::critical(this, tr("Error"),
...
void SecureMessageGUI::addEntry(QString address, QString alias)
{
    bool found = false;
    for (int i = 0; i < ui->user_list->count(); ++i) {
