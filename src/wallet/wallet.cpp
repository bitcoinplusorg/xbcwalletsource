...
CAmount CWallet::GetMinimumFee(unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool)
{
    ...
    nFeeNeeded = nFeeNeeded > DEFAULT_TRANSACTION_FEE ? nFeeNeeded : DEFAULT_TRANSACTION_FEE;
...
bool CWallet::GetStakeWeightQuick(const int64_t& nTime, const int64_t& nValue, uint64_t& nWeight) {
    arith_uint256 bnCoinDayWeight = arith_uint256(0);
    int64_t nTimeWeight;
    ...
    if(nTimeWeight > 0)
      ...
      bnCoinDayWeight = (arith_uint256(nValue) * nTimeWeight) / COIN / (24 * 60 * 60);
...
void CWallet::GetStakeWeight(uint64_t& nMinWeight, uint64_t& nMaxWeight, uint64_t& nWeight)
{
    ...
    if (!SelectCoinsForStaking(nBalance, GetTime(), COINBASE_MATURITY + COINBASE_MATURITY_OFFSET, setCoins, nValueIn))
        return;
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
                if (!(IsSpent(wtxid,i)) && IsMine(pcoin->vout[i]) && pcoin->vout[i].nValue >= nMinimumInputValue)
                    vCoins.push_back(COutput(pcoin, i, nDepth, true, (IsMine(pcoin->vout[i]) & (ISMINE_SPENDABLE)) != ISMINE_NO));
        }
    }
}
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key)
{
    ...
    if (!SelectCoinsForStaking(nBalance, txNew.nTime, COINBASE_MATURITY + COINBASE_MATURITY_OFFSET, setCoins, nValueIn))
        return false;
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && pindexPrev == pindexBestHeader; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, block, nTxPos, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake, targetProofOfStake))
            {
                ...
                if (GetWeight(pblockindex->GetBlockTime(), (int64_t)txNew.nTime) < Params().GetConsensus().nStakeSplitAge)
    ...
    if (txNew.vout.size() == 3) // 2 stake outputs, stake was split, no inode payment
    {
