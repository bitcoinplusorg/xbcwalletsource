...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:8884\",  (string) The bitcoinplus server IP and port we're connected to\n"
...
UniValue getnetworkinfo(const UniValue& params, bool fHelp)
{
    ...
    obj.push_back(Pair("subversion",    strSubVersionLocal));
...
UniValue getstakesubsidy(const UniValue& params, bool fHelp)
{
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Decode failed");
    }
